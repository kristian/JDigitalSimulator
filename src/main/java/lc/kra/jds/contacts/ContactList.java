...
public class ContactList<Type extends Contact> implements Cloneable, Serializable, Iterable<Type> {
    ...
    public static <Type extends Contact> void setContactLocations(Component component, Type[] contacts, Point location, Alignment alignment) {
        ...
        switch (alignment) {
            case HORIZONTAL:
                intervalX = (float) (component.getSize().width + 1 - location.x) / (contacts.length + 1);
                intervalY = 0;
                break;
            case VERTICAL:
                intervalX = 0;
                intervalY = (float) (component.getSize().height + 1 - location.y) / (contacts.length + 1);
                break;
        }
    ...
    @Override
    public Iterator<Type> iterator() { return contacts.iterator(); }
    ...
    @Override
    public ContactList<Type> clone() throws CloneNotSupportedException {
