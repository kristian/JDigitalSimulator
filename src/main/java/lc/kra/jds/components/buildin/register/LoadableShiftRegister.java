...
/**
 * Loadable-Shift-Register (build-in component)
 * @author Kristian Kraljic (kris@kra.lc)
 */
public class LoadableShiftRegister extends LoadableRegister {
	private static final long serialVersionUID = 2l;

	private static final String KEY;
	static { KEY = "component.register."+LoadableShiftRegister.class.getSimpleName().toLowerCase(); }
	...
	public LoadableShiftRegister() {
		inputI = new InputContact(this, new Point(0, 6));
		inputC = new InputContact(this, new Point(0, 14));
		inputL.setLocation(new Point(0, 22));
		inputR = new InputContact(this, new Point(0, 30));
		contacts = ContactUtilities.concatenateContacts(new Contact[]{inputI, inputC, inputL, inputR}, inputs.toArray(), outputs.toArray());
	}
	...
	@Override public void calculate() {
		boolean clock = inputC.isCharged(), load = inputL.isCharged();
		if(inputR.isCharged()) {
			for(OutputContact output:outputs)
				output.setCharged(false);
		} else if(!oldLoad&&load) super.calculate();
