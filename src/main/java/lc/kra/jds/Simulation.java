    ...
    private void addListeners() {
        ...
        this.addMouseListener(new MouseAdapter() {
                ...
                if (!(component instanceof Wire)) {
                    ...
                    if (component instanceof Configurable)
                        menu.add(Guitilities.createMenuItem("component.properties", new ActionListener() {
                            @Override
                            public void actionPerformed(ActionEvent actionevent) { showComponentConfigurationDialog(component); }
                        }));
                    menu.add(Guitilities.createMenuItem("component.delete", new ActionListener() {
                        @Override
                        public void actionPerformed(ActionEvent actionevent) { removeComponent(component); }
                    }));
                } else { //is a wire
            ...
            public void mousePressed(MouseEvent event) {
                last = null;
                select = current = applyZoom(event.getPoint());
                selectContact = null;
                selectWire = null;
        ...
        this.addMouseMotionListener(new MouseMotionListener() {
            ...
            public void mouseDragged(MouseEvent event) {
                ...
                last = Simulation.this.current;
                Simulation.this.current = current;
...
//    }
        ...
        for (final Option option : options) {
            ...
            if (option.hasDefault())
                if (!option.getType().equals(OptionType.FILE)) {
                    ...
                    switch (option.getType()) {
                        case LIST:
                            variable = ((Object[]) option.getDefault())[0].toString();
                            break;
                        default:
                            variable = option.getDefault().toString();
                    }
            ...
            public void actionPerformed(ActionEvent event) {
                ...
                try {
                    for (Option option : inputs.keySet()) {
                        JComponent component = inputs.get(option);
                        Object value = null;
                        switch (option.getType()) {
                            case NUMBER:
                                value = Integer.parseInt(((JSpinner) component).getValue().toString());
                                break;
                            case TEXT:
                            ...
                            case LIST:
                                ...
                                break;
                            case FILE:
                                value = new File(((JTextField) component).getText());
                                break;
                        }
    ...
    @Override
    public Dimension getPreferredSize() { return applyZoom(super.getPreferredSize()); }

    @Override
    public void paint(Graphics defaultGraphics) {
    ...
    private void paintComponents(Graphics graphics, Collection<Component> components) { paintComponents(graphics, components, null); }
    ...
    protected boolean removeListener(EventListener listener) {
    ...
    public void setComponentLayers(Collection<? extends Component> components, Layer layer) {
        ...
        switch (layer) {
            case TOPMOST:
                ...
                break;
            case BOTTOMMOST:
                data.components.addAll(0, components);
                break;
        }
        repaint();
    }
    ...
    protected boolean innerRemoveComponent(Component component) {
        ...
        if (component instanceof Sociable)
            for (Contact contact : ((Sociable) component).getContacts())
                wires.addAll(contact.getWires());
        Contact source = null;
        Collection<Contact> targets = new Vector<Contact>();
    ...
    @Override
    public int getScrollableUnitIncrement(java.awt.Rectangle rectangle, int left, int top) { return GRID_STEPS / 2; }

    @Override
    public int getScrollableBlockIncrement(java.awt.Rectangle rectangle, int left, int top) { return getScrollableUnitIncrement(rectangle, left, top) * 10; }
    ...
    @Override
    public int print(Graphics default_graphics, PageFormat pageFormat, int pageIndex) throws PrinterException {
    ...
    private List<Image> getPrintPages(PageFormat pageFormat) {
        ...
        for (int pageIndex = 0; ; pageIndex++) {
            ...
            tracker.addImage(imagePart, pageIndex + 1);
        }
    }
    ...
    public class ComponentAdded extends ComponentEvent {
        ...
        @Override
        public void undo() { Simulation.this.removeComponents(this.components); }

        @Override
        public void redo() { Simulation.this.addComponents(this.components); }

        @Override
        public boolean join(ChangeEvent event) { return false; }
    }
    ...
    public class ComponentRemoved extends ComponentEvent {
        ...
        @Override
        public void undo() { Simulation.this.addComponents(this.components); }

        @Override
        public void redo() { Simulation.this.removeComponents(this.components); }
    ...
    public class ComponentMoved extends ComponentEvent {
        ...
        public ComponentMoved(Component component, Point from, Point to) {
            super(component);
            this.from = from;
            this.to = to;
        }

        public ComponentMoved(Collection<? extends Component> components, Point from, Point to) {
            super(components);
            this.from = from;
            this.to = to;
        }
        ...
        @Override
        public void undo() { Simulation.this.moveComponents(this.components, to, from); }

        @Override
        public void redo() { Simulation.this.moveComponents(this.components, from, to); }
